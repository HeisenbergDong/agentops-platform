...
def test_full_worker_result_happy_path_reaches_project_completed(monkeypatch, tmp_path):
    ...
    _finish(db, copy, WorkerResult(command_id=copy.id, worker_id=copy.worker_id, status="success", data={"raw_text": _valid_trace()}))
...
def test_copy_latest_reply_validates_trace_and_advances_to_screenshot(monkeypatch, tmp_path):
    ...
    handle_worker_result(
        db,
        command,
        WorkerResult(command_id=command.id, worker_id=command.worker_id, status="success", data={"raw_text": trace}),
    )
...
def test_run_command_failure_marks_manual_required():
    db = _test_session()
    job, round_, command = _create_run_command_rows(db)
...
def test_browser_acceptance_success_advances_to_github_submitting():
    db = _test_session()
...
def test_browser_acceptance_missing_url_marks_manual_required():
    db = _test_session()
    job, round_, command = _create_browser_acceptance_rows(db)
...
def test_browser_acceptance_failure_marks_manual_required():
    db = _test_session()
    job, round_, command = _create_browser_acceptance_rows(db)
...
def test_git_submit_success_writes_feishu_and_completes(monkeypatch, tmp_path):
    ...
    assert round_.status == JobState.ROUND_COMPLETED
    assert round_.feishu_status == "written"
    assert written["fields"]["Trae Session ID"] == "job1-1"
